...
rtl/
  ...
  ray_march.v                 [v2] 방향벡터 따라 격자 지도를 걸어 벽까지 거리 재기 (v2.0: 고정스텝, 20x20 테스트지도)
tb/
...
**v2(레이마칭) 착수**: 첫 모듈(`ray_march.v`, 고정스텝 격자 걷기) 4가지 시나리오
(정면 벽, 경계, 대각선, 최대거리) 전부 검증 완료. 다음은 실제 트랙 크기 지도와
거리장 기반 최적화. 상세 진행 기록은 [`progress.md`](progress.md).
