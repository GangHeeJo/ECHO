// -*- verilog -*-
// tb_particle_scorer_pair_arb.v — particle_scorer_pair_arb(arbiter2로 테이블
// 포트 시분할) 기능 검증. tb_particle_scorer_pair.v와 동일한 정답지/구조.
//
//   iverilog -o sim/tb_particle_scorer_pair_arb.vvp rtl/ray_march_edt.v rtl/addr_gen.v rtl/table_mem.v rtl/sensor_pe.v rtl/arbiter2.v rtl/particle_scorer_arb.v rtl/particle_scorer_pair_arb.v tb/tb_particle_scorer_pair_arb.v
//   cd sim && vvp tb_particle_scorer_pair_arb.vvp

`timescale 1ns/1ps

module tb_particle_scorer_pair_arb;
    ...
    particle_scorer_pair_arb #(.RM_POS_W(RM_POS_W), .RD_W(RD_W), .ACC_W(ACC_W)) u_pair (
        .clk(clk), .rst_n(rst_n),
    ...
    initial begin
        ...
        $display("총 소요시간: %0.1f ns (테이블 포트 시분할이라 완전 병렬보단 느릴 수 있음 - 얼마나 느린지 확인)",
                  t_end - t_start);
